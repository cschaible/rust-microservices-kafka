...
pub fn init_producer() -> FutureProducer {
    let producer: FutureProducer = ClientConfig::new()
        ...
        .set("max.in.flight.requests.per.connection", "5") // Number of unacknowledged requests the client will send on a single connection before blocking
        .set("metadata.max.age.ms", "10000") // Period of time in milliseconds after which we force a refresh of metadata even if we haven't seen any partition leadership changes
